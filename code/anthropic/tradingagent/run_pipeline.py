...
BASESTOCK_FILE = WORKING_DIR / "basestock.json"
...
# ═══════════════════════════════════════════════════════════════════════════════
# STEP 1 – BASE STOCK SCREENER (reads basestock.json built by shard screeners)
# ═══════════════════════════════════════════════════════════════════════════════
def step1_screener() -> dict:
    """
    Loads the pre-built basestock.json (generated by screen_*_stocks.py shard
    scripts using real yfinance OHLC data). Falls back to a minimal stub if the
    file is missing or stale (> 31 days old).
    """
    print("\n=== STEP 1: BASE STOCK SCREENER (basestock.json) ===")

    month_tag = TODAY.strftime("%Y-%m")

    if BASESTOCK_FILE.exists():
        try:
            data = json.loads(BASESTOCK_FILE.read_text())
            gen_date = data.get("generated_date", "")
            if gen_date[:7] == month_tag:
                stocks = data.get("stocks", [])
                print(f"  [CACHE HIT] basestock.json is current ({gen_date}). {len(stocks)} stocks loaded.")
                return {"stocks": stocks, "from_cache": True}
            else:
                print(f"  [STALE] basestock.json is from {gen_date} (need {month_tag}). Re-run shard screeners.")
        except Exception as e:
            print(f"  [WARN] Could not read basestock.json: {e}")

    print("  [WARN] basestock.json missing or stale. Using empty universe — run screen_*_stocks.py first.")
    return {"stocks": [], "from_cache": False}
...
for pattern_notes.md (what patterns were applied, observations, confidence levels)."""
    ...
    user_msg = f"""Date: {TODAY_STR} ({day_name})
...
def step3_validator(recommendations: list) -> list:
    ...
    system_prompt = f"""You are a risk validation agent for Indian stock market recommendations.
For each stock, perform these validation checks:

a) NEGATIVE NEWS CHECK: Search your knowledge for any recent (last 7 days as of {TODAY_STR}) negative news:
...
b) US MARKET STATUS: Assess last US market session ({(TODAY.replace(day=TODAY.day-1)).isoformat() if TODAY.weekday() > 0 else (TODAY.replace(day=TODAY.day-3)).isoformat()}):
...
d) GOLD PRICE CHECK: If gold has risen >2% in last 5 days (equity stress signal):
   ...
   - Assess gold trend from your knowledge as of {TODAY_STR}
...
- Indian market context: {TODAY_STR}
- US markets: Assess the most recent US session close
...
def step5_backtest(final_recommendations: list) -> str:
    ...
    recent_recs = [r for r in existing_recs if r.get("date", "") >= cutoff]
