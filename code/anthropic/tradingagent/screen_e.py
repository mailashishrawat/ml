...
def main():
    ...
    try:
        ...
        e_syms = [s for s in all_syms if isinstance(s, str) and s.startswith('E') and len(s) <= 10]

        print(f"Found {len(e_syms)} E-stocks")
        print(f"Symbols: {e_syms}\n")
    ...
    for sym in e_syms:
        try:
            ...
            passed.append({
                ...
                'force_include': 'ETERNAL' in sym.upper()
            })
    ...
    # Save to cache
    output_path = '.cache/basestock_shard_E.json'
    with open(output_path, 'w') as f:
        json.dump(result, f, indent=2)

    print(f"\n=== RESULTS ===")
    print(f"Evaluated: {len(e_syms)}")
